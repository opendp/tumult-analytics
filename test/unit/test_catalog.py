...
def test_add_public_table():
    ...
    catalog = Catalog()
    catalog.add_private_table(source_id="public", col_types={"A": "VARCHAR"})
    assert len(catalog.tables) == 1
...
def test_invalid_addition_private_table():
    ...
    catalog = Catalog()
    source_id = "private"
    catalog.add_private_table(source_id=source_id, col_types={"A": "VARCHAR"})
    with pytest.raises(ValueError, match=f"{source_id} already exists in catalog."):
        catalog.add_private_table(source_id=source_id, col_types={"B": "VARCHAR"})
...
def test_invalid_addition_public_table():
    ...
    catalog = Catalog()
    source_id = "public"
    catalog.add_public_table(source_id, {"A": "VARCHAR"})
    with pytest.raises(ValueError, match=f"{source_id} already exists in catalog."):
        catalog.add_public_table(source_id, {"C": "VARCHAR"})
